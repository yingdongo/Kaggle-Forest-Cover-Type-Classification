...
def get_clf():
    clf=ensemble.ExtraTreesClassifier(max_features= 0.35, n_estimators= 590)
    return clf
...
def write_submit(test_ids,y_pred): 
    with open('algorithm_solution590.csv', "wb") as outfile:
        outfile.write("Id,Cover_Type\n")
...
def paramtuing_solution(train,test):
    ...
    print score
    clf=get_clf()
    y_pred=clf_predict(clf,X_train,y,X_test)
